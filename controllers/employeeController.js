...
export const updateEmployeeDetails = async (req, res) => {
  try {
    let { _id, name, age, gender, mobile, email, isMarried, isDeleted } =
      req.body;
    ...
    if (
      req.body.hasOwnProperty("isMarried") &&
...
export const deleteEmployee = async (req, res) => {
  try {
    ...
    let checkEmployeeExist = await Employee.findOne({
      _id: _id,
      isDeleted: false,
    });
    ...
    let removedEmployee = await Employee.findByIdAndUpdate(
      _id,
      { $set: {isDeleted: true}},
      { new: true }
    );
